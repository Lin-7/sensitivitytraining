...
def get_args():
    ...
    parser.add_argument('--attack-iters-begin', default=0, type=int)        # pgd训练时换回10，训练时评估用20
    parser.add_argument('--attack-iters-end', default=20, type=int)
...
def main():
    ...
    logging.basicConfig(
        ...
        level=logging.DEBUG,
        handlers=[
            logging.FileHandler(os.path.join(args.fname, 'evaluate.log'))
        ])
    ...
    logger.info('Iters \t Test Loss \t Test Acc \t Test Robust Loss \t Test Robust Acc')
    model.eval()
    for iter in range(args.attack_iters_begin, args.attack_iters_end+1):
        test_robust_acc = 0
        test_robust_loss = 0
        test_n = 0
        for _, batch in enumerate(test_batches):
            ...
            # 生成对应强度的攻击评估样本
            delta = attack_pgd(model, X, y, epsilon, pgd_alpha, iter, args.restarts, args.norm, early_stop=False)
            delta = delta.detach()
            # 使用生成的攻击样本对模型进行评估
            ...
            # 只需要评估一次模型的原始准确率和损失
            if iter == args.attack_iters_begin:
        ...
        logger.info('%d \t %.4f \t %.4f \t %.4f \t %.4f',
                iter, test_loss/test_n, test_acc/test_n, test_robust_loss/test_n, test_robust_acc/test_n)
